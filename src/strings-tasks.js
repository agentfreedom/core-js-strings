...
function removeFirstOccurrences(str, value) {
  const indexStartValue = str.indexOf(value);
  if (str === '' || value === '' || indexStartValue < 0) return str;
  const newStrStart = str.slice(0, indexStartValue);
  const newStrEnd = str.slice(indexStartValue + value.length);
  const res = newStrStart + newStrEnd;
  return res;
}
...
function removeLastOccurrences(str, value) {
  const indexStartValue = str.lastIndexOf(value);
  if (str === '' || value === '' || indexStartValue < 0) return str;
  const newStrStart = str.slice(0, indexStartValue);
  const newStrEnd = str.slice(indexStartValue + value.length);
  const res = newStrStart + newStrEnd;
...
function sumOfCodes(str) {
  let count = 0;
  if (str <= 0 || str === undefined) return count;
...
function startsWith(str, substr) {
  const res = str.startsWith(substr);
  return res;
}
...
function reverseWords(str) {
  const arr = str.split(' ');
  const reversedWordsInArr = arr.map((word) =>
    word.split('').reverse().join('')
  );
  const newStr = reversedWordsInArr.join(' ');
  return newStr;
}
...
function extractNameFromTemplate(value) {
  ...
  const lastNameStr = arr[2].split('');
  let lastName = '';
  for (let i = 0; i < lastNameStr.length; i += 1) {
    if (lastNameStr[i] !== '!') {
      lastName += lastNameStr[i];
    }
  }
